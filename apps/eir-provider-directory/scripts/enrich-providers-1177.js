...
function buildTargets(providers, options) {
  const candidates = providers
    ...
    .filter(
      (item) =>
        !options.onlyUnresolved ||
        item.provider?.services?.self_referral_verification_status === 'unresolved',
    )
...
function applyResultToProvider(provider, result) {
  ...
  if (result.status === 'ok' && result.snapshot) {
    ...
    if (snapshot.profile?.location?.latitude && snapshot.profile?.location?.longitude) {
      ...
      if (!provider.location.coordinates.lat)
        provider.location.coordinates.lat = snapshot.profile.location.latitude;
      if (!provider.location.coordinates.lng)
        provider.location.coordinates.lng = snapshot.profile.location.longitude;
    }
    if (
      !provider.contact?.phone &&
      Array.isArray(snapshot.profile?.phone) &&
      snapshot.profile.phone.length > 0
    ) {
