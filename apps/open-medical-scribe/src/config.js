...
export function loadConfig(env) {
  ...
  if (!ALLOWED_MODES.has(scribeMode)) {
    throw new Error(`Invalid SCRIBE_MODE="${scribeMode}". Expected one of: api, local, hybrid.`);
  }
  ...
  return {
    ...
    streaming: {
      transcriptionProvider:
        env.STREAMING_TRANSCRIPTION_PROVIDER ||
        defaultStreamingProvider(env.TRANSCRIPTION_PROVIDER),
