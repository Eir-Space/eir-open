    ...
    async handler(req, res) {
      try {
            ...
            supported: {
              transcription: [
                ...
                },
                {
                  ...
                  type: 'cloud',
                  configured: !!config.google.speechApiKey,
                },
                {
                  id: 'berget',
                  name: 'Berget AI (EU)',
                  type: 'cloud',
                  configured: !!config.berget.apiKey,
                },
                {
                  id: 'whisper.cpp',
                  name: 'whisper.cpp',
                  type: 'local',
                  configured: !!config.whisper.localCommand,
                },
                {
                  id: 'faster-whisper',
                  name: 'faster-whisper',
        ...
        if (req.method === 'POST' && req.url === '/v1/settings') {
          ...
          rebuildProviders();
          console.log(
            `[settings] Providers rebuilt: tx=${config.transcriptionProvider}, note=${config.noteProvider}`,
          );
          return jsonResponse(res, 200, configToSettingsResponse(config));
        }
