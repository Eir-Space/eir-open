...
export function applyModelOverride(config, providerName, model) {
  const map = {
    openai: () => {
      config.openai.transcribeModel = model;
      config.openai.noteModel = model;
    },
    anthropic: () => {
    ...
    deepgram: () => {
      config.deepgram.model = model;
    },
    google: () => {
      config.google.speechModel = model;
    },
    berget: () => {
      config.berget.transcribeModel = model;
    },
  };
