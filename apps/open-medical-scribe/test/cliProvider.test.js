...
test('cli transcription provider returns placeholder when no command', async () => {
  const { createCliTranscriptionProvider } =
    await import('../src/providers/transcription/cliProvider.js');
...
test('cli note provider returns placeholder when no command', async () => {
  const { createCliNoteGenerator } = await import('../src/providers/note/cliProvider.js');
...
test('note provider factory creates cli provider', async () => {
  const { createNoteGenerator } = await import('../src/providers/note/index.js');
...
test('cli/note.js with mock provider generates note', async () => {
  const { stdout } = await exec('node', [
    ...
    'mock',
    '--transcript',
    'Patient has cough for two days.',
  ]);
