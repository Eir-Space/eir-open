...
test('mock transcription provider returns placeholder for audio-base64', async () => {
  const { createMockTranscriptionProvider } =
    await import('../src/providers/transcription/mockProvider.js');
...
test('openai transcription provider returns unconfigured when no API key', async () => {
  const { createOpenAiTranscriptionProvider } =
    await import('../src/providers/transcription/openAiProvider.js');
...
test('deepgram transcription provider returns unconfigured when no API key', async () => {
  const { createDeepgramTranscriptionProvider } =
    await import('../src/providers/transcription/deepgramProvider.js');
...
test('google speech provider returns unconfigured when no API key', async () => {
  const { createGoogleSpeechTranscriptionProvider } =
    await import('../src/providers/transcription/googleSpeechProvider.js');
...
test('berget transcription provider returns unconfigured when no API key', async () => {
  const { createBergetTranscriptionProvider } =
    await import('../src/providers/transcription/bergetProvider.js');
...
test('whisper.cpp provider returns placeholder when no local command configured', async () => {
  const { createWhisperCppTranscriptionProvider } =
    await import('../src/providers/transcription/whisperCppProvider.js');
...
test('transcription provider factory creates all supported providers', async () => {
  const { createTranscriptionProvider } = await import('../src/providers/transcription/index.js');
