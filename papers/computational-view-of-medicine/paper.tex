...
Psychiatry is computational because the organ of interest is computationally complex. The relevant clinical state is not captured by a single laboratory value or imaging slice; it is distributed across history, context, behavior, sleep, substance use, prior treatment response, and the patient's own account of what changes their internal state. A clinician deciding what to do next in depression, bipolar disorder, ADHD, or anxiety is therefore working with a partially observed system whose state has to be inferred from a noisy longitudinal record.
...
The same structure appears in psychotherapy. Cognitive behavioral therapy is, in practice, an effort to map the loops that keep a disorder stable: triggers, beliefs, behaviors, and reinforcing consequences \citep{beck2005}. That is close to an action-selection problem over state. A therapist is trying to identify which intervention is most likely to move the patient from a maladaptive local equilibrium toward a better one, given the current pattern of thought, behavior, and context. Under a computational view, psychiatry belongs in the portfolio of high-burden reasoning problems precisely because more context, better state estimation, and better action selection can change the next therapeutic move.

\subsection{Patient-reported outcomes and symptom monitoring}
Patient-reported outcomes provide a useful bridge case. They show that high-value computation is not limited to genomics, molecular medicine, or the ICU. Electronic patient-reported symptom monitoring during cancer treatment improved overall survival, likely by enabling earlier response to deterioration patterns otherwise missed \citep{basch2016,basch2017}. Symptoms, side effects, and tolerability live outside the hospital until severe enough to trigger contact. In the computational paradigm, those signals become first-class inputs into the case model. The wider point is that once more of the clinically relevant state lives outside the visit, systems that can repeatedly integrate those signals become more valuable.
