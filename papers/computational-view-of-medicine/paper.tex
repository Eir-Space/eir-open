...
The same structure appears in psychotherapy, but it can be stated more cleanly in computational-neuroscience terms. On one influential view, the brain is continuously generating predictions, updating them from error signals, and selecting actions that reduce uncertainty and improve expected outcomes \citep{friston2010,schultz2016}. Psychiatric illness can then be understood not simply as a set of ``symptoms,'' but as a disturbance in inference, valuation, and policy selection expressed through mood, attention, reward, belief, and behavior. Tolstoy's line that ``every unhappy family is unhappy in its own way'' captures something clinically relevant here: psychiatric illness is often idiosyncratic in its exact configuration, even when it resembles a familiar syndrome. The therapeutic task is therefore to infer, as well as possible, the patient's current model of the world and self, identify where it is maladaptive or unstable, and choose the next intervention most likely to move the system toward a better trajectory \citep{beck2005,carhart2014}. Under a computational view, psychiatry belongs in the portfolio of high-burden reasoning problems precisely because more context, better within-person modeling, and better action selection can change the next therapeutic move.
